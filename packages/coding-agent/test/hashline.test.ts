...
type Tool = {
	...
	execute: (
		id: string,
		params: unknown,
	) => Promise<{ content: Array<{ text: string }>; details?: { error?: boolean } }>;
};
...
beforeAll(async () => {
	// hashline is opt-in (off by default) — enable it before importing so the
	// module reads the flag at eval time and registers its tools.
	process.env.KP_HASHLINE_ENABLED = "1";
...
test("hread returns HASH│content and hedit replaces a range by anchor", async () => {
	...
	expect(edit.details?.error).toBeFalsy();
...
test("hedit_block replaces a whole brace construct by its start anchor (JS fallback path)", async () => {
	...
	expect(r.details?.error).toBeFalsy();
