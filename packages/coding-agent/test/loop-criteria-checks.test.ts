...
test("all criteria fail when the work isn't there yet", () => {
	const r = runCriteriaChecks(loop());
...
test("criteria flip to pass once the work objectively satisfies them", () => {
...
test("partial: one pass, one fail is reported precisely", () => {
...
test("a model-set pass is OVERRULED by the real check (no self-assessment)", () => {
